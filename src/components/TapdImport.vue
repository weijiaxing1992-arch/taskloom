...
import { t } from '../i18n'
import { buildTapdRequirement, suggestTapdMapping, tapdFileError, tapdMemberIssue, tapdDocumentError, type TapdMapping, type TapdTarget } from '../tapdImport'
import { fieldMemberRoles, memberCandidates } from '../mentions'
import { plainTextToDocument, richTextPlain } from '../richText'
...
const members=ref<any[]>([]),targets=ref<TapdTarget[]>([]),rows=ref<TapdMapping[]>([]),title=ref(''),description=ref(''),fileInput=ref<HTMLInputElement|null>(null)
const descriptionDoc=ref<JSONContent|null>(null),pdfUrl=ref(''),mediaBusy=ref(false)
const layoutWarnings=computed(()=>parsed.value?.warnings.filter(warning=>/^(?:第 \d+ 页|未提取|正文可能|复杂版式)/.test(warning))||[])
let controller:AbortController|null=null,version=0,disposed=false
const dragDepth=ref(0),tab=ref<'fields'|'body'>('fields'),showArchived=ref(false),onlyIssues=ref(false)
const fileDisabled=computed(()=>loading.value||saving.value||mediaBusy.value||!targets.value.length||scope.locked.value)
const draftDisabled=computed(()=>loading.value||saving.value||scope.locked.value)
...
function releasePdf(){if(pdfUrl.value)URL.revokeObjectURL(pdfUrl.value);pdfUrl.value=''}
function close(){if(saving.value||mediaBusy.value)return;stop();releasePdf();opened.value=false;parsed.value=null;descriptionDoc.value=null;rows.value=[];error.value=''}
async function open(){
  if(opened.value||saving.value||mediaBusy.value||!scope.current())return
  stop();releasePdf();opened.value=true;notice.value='';error.value='';parsed.value=null;descriptionDoc.value=null;rows.value=[];targets.value=[];members.value=[];reviewed.value=false;loading.value=true
  ...
  try{
    ...
    if(!states.canManage)throw Error('仅项目管理员或企业管理员可以迁移需求')
    ...
    const definitions:TapdTarget[]=[
      ...
      {key:'ownerUserIds',label:'产品负责人',kind:'members',memberRoles:fieldMemberRoles({key:'ownerUserIds'})},{key:'assigneeUserIds',label:'处理人',kind:'members'},
      ...(['frontend','backend','algorithm','ui','product'] as const).flatMap((key,index)=>[
        {key:`role.${key}.members`,label:['前端工程师','后端工程师','算法工程师','UI设计师','产品人员'][index]!,kind:'members' as const,memberRoles:fieldMemberRoles({key:`role.${key}.userIds`})},
      ...
      ...fields.items.filter((f:any)=>f.enabled&&['text','textarea','number','boolean','date','select','user','users'].includes(f.type)).map((f:any)=>({key:'cf.'+f.key,label:f.name,kind:({select:'choice',user:'members',users:'members',textarea:'text'} as any)[f.type]||f.type,single:f.type==='user',memberRoles:fieldMemberRoles(f),departmentId:f.departmentId,options:(f.options||[]).map((value:string)=>({value,label:value}))})),
    ]
    targets.value=definitions.map(target=>target.kind==='members'?{...target,memberIds:memberCandidates(members.value,target.memberRoles,target.departmentId).map(member=>member.id)}:target)
...
async function selectFiles(files:File[]){
  if(fileDisabled.value||mediaBusy.value||!scope.current())return
  ...
  // Replacement is transactional: a corrupt/unsupported PDF must not discard the reviewed draft.
  loading.value=true;error.value='';progress.value=''
  try{
    ...
    if(ticket!==version||!scope.current())return
    const nextDoc=result.descriptionDoc||plainTextToDocument(result.description),contentError=tapdDocumentError(nextDoc,richTextPlain(nextDoc))
    if(contentError)throw Error(contentError)
    const nextRows=result.fields.map(field=>suggestTapdMapping(field,targets.value,members.value)),nextUrl=URL.createObjectURL(file)
    releasePdf();parsed.value=result;title.value=result.title;description.value=result.description;descriptionDoc.value=nextDoc;pdfUrl.value=nextUrl;reviewed.value=false
    tab.value='fields';showArchived.value=false;onlyIssues.value=false
    rows.value=nextRows
  }catch(cause){if(ticket===version&&scope.current())error.value=cause instanceof Error?cause.message:'PDF 解析失败，请检查文件'}finally{if(ticket===version){loading.value=false;controller=null;if(fileInput.value)fileInput.value.value=''}}
}
...
function changed(row:TapdMapping){row.issue=tapdMemberIssue(row.value,targetOf(row));reviewed.value=false}
...
async function submit(){
  if(saving.value||loading.value||mediaBusy.value||!parsed.value||!reviewed.value||!scope.current())return
  const ticket=version
  error.value=''
  try{
    ...
    const doc=descriptionDoc.value||plainTextToDocument(description.value)
    const contentError=tapdDocumentError(doc,richTextPlain(doc))
    if(contentError)throw Error(contentError)
    const {descriptionDoc:originalDoc,imageCount,description:originalDescription,title:originalTitle,displayId,...source}=parsed.value
    ...
    if(disposed||ticket!==version||!scope.current())return
  ...
  }catch(cause){if(!disposed&&ticket===version&&scope.current())error.value=cause instanceof Error?cause.message:'导入失败，请重试'}finally{saving.value=false}
}
watch(scope.locked,locked=>{if(locked){stop();releasePdf();parsed.value=null;descriptionDoc.value=null;rows.value=[];reviewed.value=false}}, {flush:'sync'})
onBeforeUnmount(()=>{disposed=true;stop();releasePdf()})
</script>

<template>
 <div class="tapd-entry"><button type="button" class="btn" :disabled="scope.locked.value||opened" @click="open">{{t('TAPD 迁移')}}</button><span v-if="notice" role="status" class="tapd-notice">{{notice}}</span></div>
 <OrganizationModal v-if="opened" :title="t('TAPD PDF 迁移')" wide :busy="saving" @close="close">
  <div class="tapd-content" :aria-busy="loading||saving">
   ...
   <template v-if="parsed">
    <label class="tapd-title"><span>{{t('需求标题')}}</span><input v-model="title" :disabled="draftDisabled" maxlength="300" @input="reviewed=false"/></label>
    <div class="tapd-tabs" role="tablist" :aria-label="t('导入预览')">
     ...
     <button type="button" id="tapd-body-tab" role="tab" :aria-selected="tab==='body'" aria-controls="tapd-body-panel" @click="tab='body'"><Icon name="review"/>{{t('正文与原图')}}</button>
    ...
    <section v-show="tab==='fields'" id="tapd-fields-panel" role="tabpanel" aria-labelledby="tapd-fields-tab">
     ...
     <div class="tapd-fields" role="group" :aria-label="t('字段对照')">
      <div v-for="{row,index} in shownRows" :key="index" class="tapd-field" :class="{'has-issue':row.target&&row.issue}">
       ...
       <AppSelect :model-value="row.target" :options="targetOptions" :label="t('目标字段')+' '+row.field.label" :disabled="draftDisabled" @update:model-value="setTarget(row,$event)"/>
       <div class="tapd-value">
        <template v-if="row.target">
         <MemberMultiSelect v-if="targetOf(row)?.kind==='members'" v-model="row.value" :members="members" :member-roles="targetOf(row)?.memberRoles" :department-id="targetOf(row)?.departmentId" :single="targetOf(row)?.single" :show-lead="false" :label="row.field.label" compact :disabled="draftDisabled" @update:model-value="changed(row)"/>
         <AppSelect v-else-if="targetOf(row)?.kind==='choice'" v-model="row.value" :options="choiceOptions(row)" :label="row.field.label" :disabled="draftDisabled" @update:model-value="changed(row)"/>
         <AppSelect v-else-if="targetOf(row)?.kind==='boolean'" v-model="row.value" :options="[{value:'',label:t('请选择目标值')},{value:'true',label:t('是')},{value:'false',label:t('否')}]" :label="row.field.label" :disabled="draftDisabled" @update:model-value="changed(row)"/>
         <input v-else v-model="row.value" :type="targetOf(row)?.kind==='number'?'number':targetOf(row)?.kind==='date'?'date':'text'" :aria-label="row.field.label" :disabled="draftDisabled" @input="changed(row)"/>
     ...
     <div class="tapd-table-footer"><span>{{rows.filter(row=>!row.target).length}} {{t('个未映射字段已保留，不会丢失')}}</span><button type="button" class="btn" :disabled="draftDisabled" @click="addRow"><Icon name="plus" :size="14"/>{{t('补充目标字段')}}</button></div>
    </section>
    <section v-show="tab==='body'" id="tapd-body-panel" role="tabpanel" aria-labelledby="tapd-body-tab" class="tapd-body-grid">
     <div class="tapd-description"><span>{{t('可编辑正文')}} · {{parsed.imageCount||0}} {{t('张独立原图')}}</span><RichTextEditor :key="pdfUrl" v-model="description" v-model:document="descriptionDoc" :members="[]" :disabled="draftDisabled" :label="t('可编辑正文')" @update:document="reviewed=false" @busy="mediaBusy=$event"/></div>
     <div class="tapd-original"><div class="tapd-pages"><strong>{{t('原始 PDF')}}</strong><a :href="pdfUrl" target="_blank" rel="noopener">{{t('打开原始 PDF 核对')}}</a></div><object :data="pdfUrl" type="application/pdf" :aria-label="t('原始 PDF 页面预览')"><a :href="pdfUrl" target="_blank" rel="noopener">{{t('打开原始 PDF 核对')}}</a></object></div>
    </section>
    <div v-if="layoutWarnings.length" class="tapd-layout-warning" role="status"><strong>{{t('版式核对提示')}}</strong><ul><li v-for="warning in layoutWarnings" :key="warning">{{t(warning)}}</li></ul></div>
    <p class="tapd-privacy">{{t('管理员可将历史需求迁移至当前项目的已启用状态，并记录 TAPD 导入审计；普通创建仍仅支持初始状态或草稿。来源状态始终留档。')}}</p>
  ...
  <template #footer><div class="tapd-footer"><label v-if="parsed" class="tapd-confirm" :title="t('已核对所有字段、正文与页面预览，同意按以上映射创建需求')"><input v-model="reviewed" type="checkbox" :disabled="draftDisabled||mediaBusy"/>{{t('已核对字段与正文')}}</label><span v-else class="tapd-footer-note">{{t('原 PDF 与字段对照将随需求保存')}}</span><div class="tapd-actions"><button type="button" class="btn" :disabled="saving||mediaBusy" @click="close">{{t('取消')}}</button><button type="button" class="btn primary" :disabled="!parsed||!reviewed||unresolved>0||loading||saving||scope.locked.value||mediaBusy" @click="submit">{{t(saving?'正在导入…':'确认导入需求')}}</button></div></div></template>
 </OrganizationModal>
</template>
...
.tapd-original object{display:block;width:100%;height:520px;border:1px solid var(--line);border-radius:4px}.tapd-description{min-width:0}.tapd-description :deep(.rich-editor){color:var(--ink)}.tapd-layout-warning{margin-top:14px;padding:12px;border:1px solid var(--line);border-radius:6px;background:var(--surface);font-size:12px;line-height:1.7}.tapd-layout-warning ul{padding-left:18px;margin:6px 0 0}
