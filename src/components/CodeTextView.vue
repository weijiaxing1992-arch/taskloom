...
const parts=computed(()=>splitCodeFences(props.text)),message=ref(''),wrap=ref(false)
...
<template><div class="code-text-view"><template v-for="(part,index) in parts" :key="index"><section v-if="part.kind==='code'" class="code-reading" :class="{'code-wrap':wrap}"><div class="code-reading-tools"><span>{{t(label(part.language,part.text))}}</span><span>{{t('{count} 行',{count:part.text.split('\n').length})}}</span><button type="button" :aria-pressed="wrap" @click="wrap=!wrap">{{t('自动换行')}}</button><button type="button" @click="copy(part.text)">{{t('复制代码')}}</button></div><pre><Highlight :code="part.text" :language="part.language"/></pre></section><div v-else class="code-plain-text">{{part.text}}</div></template><span v-if="message" class="code-copy-status" role="status">{{t(message)}}</span></div></template>
