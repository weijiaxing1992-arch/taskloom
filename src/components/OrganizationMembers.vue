...
function open(member?:OrganizationMember){
  ...
  Object.assign(form,{name:member?.name||'',email:member?.email||'',employeeNo:member?.employeeNo||'',initialPassword:'',tenantRole:member?.tenantRole||'member',active:member?.active??true,departmentIds,primaryDepartmentId,projectMemberships:member?.projectMemberships.map(item=>({...item}))||[]})
...
function prepareImpersonation(member:OrganizationMember){if(webhookMember.value||bulkAction.value)return;impersonating.value=member;reason.value=defaultImpersonationReason;error.value=''}
...
<template>
  ...
  <OrganizationModal v-if="opened" :title="t(editing?'编辑成员':'添加成员')" :busy="saving" wide @close="close"><form id="org-member-form" class="org-form" :inert="saving" @submit.prevent="save"><div class="org-form-two"><label>{{t('姓名')}} *<input v-model="form.name" required maxlength="80"></label><label>{{t('登录邮箱')}} *<input v-model="form.email" required type="email" maxlength="254"></label><label>{{t('工号')}}<input v-model="form.employeeNo" maxlength="80"></label><label>{{t(editing?'设置新密码（留空不修改）':'初始密码')}}<input v-model="form.initialPassword" type="password" autocomplete="new-password" minlength="6" maxlength="72" :required="!editing&&form.active&&!context.initialPasswordConfigured"><small v-if="!editing&&context.initialPasswordConfigured">{{t('留空使用系统初始密码，首次登录后必须修改。')}}</small></label></div><label>{{t('所属部门')}}<div class="org-multi"><label v-for="item in departmentOptions.filter(item=>item.status==='active'||form.departmentIds.includes(item.id))" :key="item.id" class="org-check"><input v-model="form.departmentIds" type="checkbox" :value="item.id">{{departmentPath(item,departments)}}</label><small v-if="!departments.length">{{t('请先创建部门')}}</small></div></label><label v-if="form.departmentIds.length">{{t('主部门')}}<select v-model="form.primaryDepartmentId" required><option v-for="id in primaryDepartmentOptions" :key="id" :value="id">{{departmentNames([id])}}</option></select></label><div class="org-form-two"><label v-if="context.isTenantAdmin">{{t('企业角色')}}<select v-model="form.tenantRole"><option value="member">{{t('普通成员')}}</option><option value="tenant_admin">{{t('企业管理员')}}</option></select></label><label class="org-check"><input v-model="form.active" type="checkbox" :disabled="editing?.id===session?.user?.id">{{t('激活此账号')}}</label></div><fieldset><legend>{{t('项目角色')}}</legend><div v-for="(membership,index) in form.projectMemberships" :key="index" class="org-project-role"><select v-model="membership.projectId" required :aria-label="t('项目空间')"><option value="">{{t('选择项目')}}</option><option v-for="project in context.projects" :key="project.id" :value="project.id" :disabled="form.projectMemberships.some((item,i)=>i!==index&&item.projectId===project.id)">{{project.name}}</option></select><select v-model="membership.role" required :aria-label="t('项目角色')"><option v-for="role in projectRoleOptions(membership)" :key="role.key" :value="role.key">{{t(role.name)}}</option></select><button type="button" :aria-label="t('移除项目权限')" @click="form.projectMemberships.splice(index,1)">×</button></div><Button type="button" size="sm" variant="outline" @click="form.projectMemberships.push({projectId:'',role:'viewer'})">＋ {{t('添加项目权限')}}</Button><p class="org-note">{{t('不选择项目时，普通成员无法访问项目内的需求与缺陷。')}}</p></fieldset><p v-if="error" class="org-error" role="alert">{{t(error)}}</p></form><template #footer>
