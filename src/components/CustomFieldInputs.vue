...
const props = defineProps<{ objectType: string; modelValue: Record<string, any>; inline?: boolean; visibleKeys?: string[] | null; excludedKeys?: string[]; excludedNames?: string[] }>()
...
const visibleDefs = computed(() => {
  ...
  return candidates.filter(definition => !keys.has(definition.key) && !names.has(definition.name))
})
...
async function load() {
  ...
  const options = { headers: { 'X-DevFlow-Project': project } }
...
function setNumber(definition: FieldDefinition, event: Event) {
  ...
  if (value === null || Number.isFinite(value)) set(definition.key, value)
}
...
<template>
  <div :class="['custom-fields', inline ? 'inline' : '']" :aria-busy="loading">
    ...
    <template v-for="definition in visibleDefs" :key="definition.id">
      ...
      <select v-if="definition.type === 'single_select'" :id="fieldID(definition)" :value="valueOf(definition) ?? ''" :aria-label="definition.name" :aria-required="!!definition.required" @change="set(definition.key, ($event.target as HTMLSelectElement).value)">
      ...
      <div v-else-if="definition.type === 'multi_select'" class="option-checks" role="group" :aria-labelledby="labelID(definition)">
        <label v-for="(option, index) in optionsOf(definition)" :key="option" :for="fieldID(definition) + '-option-' + index"><input :id="fieldID(definition) + '-option-' + index" type="checkbox" :aria-label="definition.name + '：' + option" :checked="selectedOptions(definition).includes(option)" @change="toggleOption(definition, option, $event)"> {{ option }}</label>
        <span v-if="!optionsOf(definition).length" class="custom-field-empty">{{ t('暂无可选项') }}</span>
      </div>
      <label v-else-if="definition.type === 'boolean'" class="check" :for="fieldID(definition)"><input :id="fieldID(definition)" type="checkbox" :aria-label="definition.name" :aria-required="!!definition.required" :checked="valueOf(definition) === true" @change="set(definition.key, ($event.target as HTMLInputElement).checked)"> {{ t('是') }}</label>
      <input v-else-if="definition.type === 'number'" :id="fieldID(definition)" type="number" step="any" inputmode="decimal" :aria-label="definition.name" :aria-required="!!definition.required" :value="valueOf(definition) ?? ''" @input="setNumber(definition, $event)">
      <DatePicker v-else-if="definition.type === 'date'" :id="fieldID(definition)" :label="definition.name" :required="!!definition.required" :model-value="valueOf(definition) ?? ''" @update:model-value="set(definition.key, $event)" @validity-change="dateValidity[definition.key] = $event" />
      <MemberMultiSelect v-else-if="definition.type === 'user'" single :input-id="fieldID(definition)" :label="definition.name" :model-value="valueOf(definition)?[String(valueOf(definition))]:[]" :members="members" :department-id="definition.departmentId" :member-roles="rolesFor(definition)" :disabled="loading || !!memberError" :show-lead="false" @update:model-value="set(definition.key, $event[0]||'')" />
      <MemberMultiSelect v-else-if="definition.type === 'users'" :input-id="fieldID(definition)" :label="definition.name" :model-value="selectedOptions(definition)" :members="members" :department-id="definition.departmentId" :member-roles="rolesFor(definition)" :disabled="loading || !!memberError" :show-lead="false" :hint="t('可选多位成员；仅可新增当前项目与限定部门中的可用成员，历史绑定保留。')" @update:model-value="set(definition.key, $event)" />
      <textarea v-else-if="definition.type === 'textarea'" :id="fieldID(definition)" :aria-label="definition.name" :aria-required="!!definition.required" :value="valueOf(definition) ?? ''" @input="set(definition.key, ($event.target as HTMLTextAreaElement).value)"></textarea>
      <input v-else :id="fieldID(definition)" :aria-label="definition.name" :aria-required="!!definition.required" :value="valueOf(definition) ?? ''" @input="set(definition.key, ($event.target as HTMLInputElement).value)">
    </template>
  </div>
</template>
