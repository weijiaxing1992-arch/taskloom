...
import { searchItemsInProject, topSearchTarget, searchDisplayCode, searchStatusLabel, type TopSearchItem } from '../topSearch'
...
const recentIndex=ref(-1)
const showingRecent=computed(()=>opened.value&&!query.value.trim()&&recentQueries.value.length>0)
...
async function search(){
 ...
 recentQueries.value=rememberRecentSearch(typeof window==='undefined'?undefined:window.localStorage,scopeKey.value,text)
...
function openPopup(){if(!available.value)return;opened.value=true;recentIndex.value=-1;recentQueries.value=readRecentSearches(typeof window==='undefined'?undefined:window.localStorage,scopeKey.value);if(query.value.trim()&&appliedQuery!==query.value.trim())schedule()}
function chooseRecent(queryText:string){if(!available.value)return;query.value=queryText;void nextTick(()=>search())}
...
function keydown(event:KeyboardEvent){
 ...
 if(showingRecent.value){
  if(event.key==='ArrowDown'||event.key==='ArrowUp'){event.preventDefault();const length=recentQueries.value.length;recentIndex.value=recentIndex.value<0?(event.key==='ArrowDown'?0:length-1):(recentIndex.value+(event.key==='ArrowDown'?1:-1)+length)%length;return}
  if(event.key==='Enter'&&recentIndex.value>=0){event.preventDefault();chooseRecent(recentQueries.value[recentIndex.value]!);return}
 }
...
watch(scopeKey,()=>{invalidated.value=false;clearMemory();recentQueries.value=readRecentSearches(typeof window==='undefined'?undefined:window.localStorage,scopeKey.value)},{flush:'sync'})
...
<template>
 <div ref="root" class="top-search-widget" :data-open="opened&&available" @focusout="focusout">
  <div class="top-search-input"><Icon name="search" :size="15"/><input ref="input" v-model="query" maxlength="200" :disabled="!available" role="combobox" aria-autocomplete="list" :aria-expanded="opened" :aria-controls="showingRecent?listID+'-recent':listID" :aria-activedescendant="showingRecent&&recentIndex>=0?listID+'-recent-'+recentIndex:opened&&items[activeIndex]?listID+'-'+activeIndex:undefined" :aria-label="t('全局搜索')" :placeholder="t('搜索内容、人员或部门…')" @focus="openPopup" @keydown="keydown" @compositionstart="composing=true;stopRequest()" @compositionend="composing=false;schedule()"><button v-if="query" type="button" :disabled="!available||navigating" :aria-label="t('清空搜索')" @click="clear">×</button><kbd v-else>⌘ K</kbd></div>
  <section v-if="opened&&available" class="top-search-popup" :aria-label="t('搜索建议')" :aria-busy="loading||navigating">
   ...
   <div v-if="!query.trim()" :id="listID+'-recent'" class="top-search-recent" role="listbox" :aria-label="t('最近搜索')"><p>{{t('最近搜索')}}</p><button v-for="(item,index) in recentQueries" :id="listID+'-recent-'+index" :key="item" type="button" role="option" :aria-selected="recentIndex===index" :class="{active:recentIndex===index}" @mouseenter="recentIndex=index" @mousedown.prevent @click="chooseRecent(item)"><Icon name="search" :size="13"/><span>{{item}}</span></button><small v-if="!recentQueries.length">{{t('输入标题、编号、负责人、开发人员或部门名称，即时查找相关事项。')}}</small></div>
   ...
   <div :id="listID" class="top-search-results" role="listbox" :aria-label="t('搜索结果')">
    <button v-for="(item,index) in items" :id="listID+'-'+index" :key="item.type+'-'+item.id" type="button" role="option" :aria-selected="activeIndex===index" :disabled="navigating||loading" :class="{active:activeIndex===index}" @mouseenter="activeIndex=index" @mousedown.prevent @click="choose(item)"><span class="top-search-copy"><span class="top-search-result-tags"><span class="top-search-type" :class="{'is-defect':item.type==='缺陷'}">{{t(item.type)}}</span><span v-if="item.status" class="top-search-status workflow-color" :style="workflowStyle(item)">{{searchStatusLabel(item,t)}}</span><small class="top-search-code">{{searchDisplayCode(item)}}</small></span><b>{{item.title}}</b><small v-if="item.snippet">{{item.snippet}}</small></span></button>
   </div>
...
<style scoped>
.top-search-recent{display:grid;gap:3px;padding:10px 8px}.top-search-recent>p{margin:0;padding:3px 7px 6px;color:var(--muted);font-size:11px}.top-search-recent>small{padding:11px 8px 14px;color:var(--muted);font-size:12px;line-height:1.6}.top-search-recent>button{display:flex;align-items:center;gap:8px;min-width:0;border:0;border-radius:7px;background:transparent;padding:9px 8px;color:var(--ink);font:inherit;font-size:12px;text-align:left}.top-search-recent>button:hover{background:var(--primary-soft)}.top-search-recent>button span{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
</style>
