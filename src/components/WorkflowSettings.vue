...
<template>
  <section class="workflow-settings" :aria-busy="loading || saving">
    ...
    <div v-if="editor && selectedEdge" class="settings-modal-shade slide-panel-shade" @click.self="closeRole()" @keydown="closeRole"><section ref="modal" tabindex="-1" class="settings-modal workflow-role-modal" role="dialog" aria-modal="true" :aria-label="t('流转角色权限')"><header><div><h2>{{ t('流转角色权限') }}</h2><p>{{ statusName(editor.from) }} → {{ statusName(editor.to) }}</p></div><button type="button" :disabled="saving" :aria-label="t('关闭')" @click="closeRole()">×</button></header><div class="workflow-role-list"><label v-for="role in permittedRoles" :key="role.key"><input type="checkbox" :disabled="!mutable" :checked="selectedEdge.roles.includes(role.key)" @change="toggleRole(role.key)">{{ t(role.name) }}<code>{{ role.key }}</code></label></div><p class="settings-note">{{ t('至少选择一个角色。此处修改加入工作流草稿，点击“保存工作流”后生效。') }}</p><p v-if="!selectedEdge.roles.length" class="settings-warning">{{ t('尚未选择允许的角色') }}</p><footer><button type="button" class="btn primary" :disabled="saving" @click="closeRole()">{{ t('完成配置') }}</button></footer></section></div>
  </section>
</template>
