...
type Member = { id: string; name: string; email: string; active: boolean; tenantRole: string; role: string | null; projectRoles?:string[] }
...
const roleDrafts = ref<Record<string,string[]>>({}), roleBaselines = ref<Record<string,string[]>>({})
const projectRoleKeys=['project_admin','product','frontend','backend','algorithm','ui','frontend_lead','backend_lead','qa','viewer']
const roleUpdates=computed(()=>Object.fromEntries(items.value.filter(item=>selectedSet.value.has(item.id)&&JSON.stringify([...(roleDrafts.value[item.id]??[])].sort())!==JSON.stringify([...(roleBaselines.value[item.id]??[])].sort())).map(item=>[item.id,[...(roleDrafts.value[item.id]??[])]])))
...
const dirty = computed(() => added.value.length > 0 || removed.value.length > 0 || Object.keys(roleUpdates.value).length>0)
...
function editableRoles(item:Member){return isTenantAdmin.value||item.tenantRole!=='tenant_admin'&&!['project_admin','tenant_admin'].some(role=>(item.projectRoles??[item.role]).includes(role))}
...
function acceptCandidates(result: Candidates) {
  ...
  roleDrafts.value=Object.fromEntries(items.value.map(item=>[item.id,[...(item.projectRoles?.length?item.projectRoles:[item.role||'viewer'])]]))
  roleBaselines.value=Object.fromEntries(Object.entries(roleDrafts.value).map(([id,roles])=>[id,[...roles]]))
}
...
async function load() {
  ...
  try {
    const result = await api<Candidates>(`/projects/${encodeURIComponent(projectId)}/members?candidates=1`, { headers: { 'X-TaskLoom-Project': projectId }, signal: controller.signal })
...
async function save() {
  ...
  const changedRoles = roleUpdates.value
  if(Object.values(changedRoles).some(roles=>!roles.length)){error.value='至少选择一个项目角色';return}
  if(new Set([...addUserIds,...removeUserIds,...Object.keys(changedRoles)]).size>200){error.value='每次最多调整 200 位成员，请分批保存';return}
  ...
  try {
    const result = await api<Candidates>(`/projects/${encodeURIComponent(projectId)}/members`, { method: 'PATCH', headers: { 'X-TaskLoom-Project': projectId }, signal: controller.signal, body: JSON.stringify({ addUserIds, removeUserIds, role: 'viewer', ...(Object.keys(changedRoles).length?{roleUpdates:changedRoles}:{}) }) })
    if (!current(version, projectId)) return
    acceptCandidates(result)
...
function discardChanges() {
  ...
  selected.value = [...baseline.value]; roleDrafts.value=Object.fromEntries(Object.entries(roleBaselines.value).map(([id,roles])=>[id,[...roles]])); discardOpen.value = false; settleLeave(true); emit('close')
}
...
<template>
  <OrganizationModal :title="t('管理项目成员')" :busy="saving" wide @close="requestClose">
    <div class="project-members" :aria-busy="loading || saving">
      ...
      <p class="members-help">{{ t('新增成员默认只读，可展开编辑并多选项目角色。首次登录前也可完成项目授权。') }}</p>
      ...
      <template v-if="loaded">
        ...
        <div class="members-list">
          <div v-for="item in filtered" :key="item.id" class="members-row" :class="{ selected: selectedSet.has(item.id), protected: protectedMember(item) }">
            ...
            <div class="members-meta"><span v-if="!item.active" class="members-badge inactive">{{ t('未激活') }}</span><span v-if="item.tenantRole === 'tenant_admin'" class="members-badge">{{ t('企业管理员') }}</span><span v-if="item.role" class="members-role">{{ (roleDrafts[item.id]??[]).map(roleLabel).join(' / ') }}</span><span class="members-membership" :class="{ added: selectedSet.has(item.id) && !baselineSet.has(item.id), removed: !selectedSet.has(item.id) && baselineSet.has(item.id) }">{{ t(baselineSet.has(item.id) ? selectedSet.has(item.id) ? '已加入' : '待移除' : selectedSet.has(item.id) ? '待加入' : '未加入') }}</span><details v-if="selectedSet.has(item.id)&&editableRoles(item)" class="members-role-editor"><summary>{{t('编辑角色')}}</summary><div><label v-for="key in projectRoleKeys.filter(key=>isTenantAdmin||key!=='project_admin')" :key="key"><input v-model="roleDrafts[item.id]" type="checkbox" :value="key" :disabled="!mutable">{{roleLabel(key)}}</label></div></details></div>
          </div>
...
<style scoped>
.members-role-editor{width:100%;font-size:12px}.members-role-editor summary{cursor:pointer;color:var(--primary);padding:5px 0}.members-role-editor>div{display:flex;flex-wrap:wrap;gap:8px 12px;padding:8px 0}.members-role-editor label{display:flex;gap:5px;align-items:center}
