...
<template>
 <section class="workload-trends" :aria-label="t('工作量趋势')">
  ...
  <div class="trend-filters"><label><span>{{t('所属部门')}}</span><AppSelect class="trend-select" :model-value="department" :options="departmentOptions" :label="t('所属部门')" :disabled="blocked" @update:model-value="selectDepartment"/></label><div class="trend-member-filter"><span>{{t('成员')}}</span><MemberMultiSelect compact single :show-lead="false" :model-value="member?[member]:[]" :members="filterMembers" :all-departments="options.departments" :disabled="blocked" :label="t('筛选成员（含历史成员）')" :hint="t('所有成员')" @update:model-value="selectMember"/></div><label><span>{{t('职能')}}</span><AppSelect class="trend-select" :model-value="role" :options="roleOptions" :label="t('职能')" :disabled="blocked" @update:model-value="selectRole"/></label><Button v-if="department!=='*'||member||role" variant="ghost" size="sm" :disabled="blocked" @click="reset">{{t('清除筛选')}}</Button></div>
...
<style scoped>
.trend-member-filter{display:flex;align-items:center;gap:8px;min-width:220px;max-width:360px;flex:1;color:var(--muted);font-size:12px}.trend-member-filter>span{flex:none}.trend-member-filter:deep(.member-multi){min-width:150px;flex:1}.trend-member-filter:deep(.member-filter-control){height:34px}
