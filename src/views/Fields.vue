...
// 排序只用于兼容历史展示，不再作为用户可编辑字段发送。
function fieldPayload(field:Field) { const {sortOrder,...payload}=field;return payload }
...
async function save() {
  ...
  try{const result=await scope.request<Field>(editing.value?'/field-definitions/'+editing.value.id:'/field-definitions',{method:editing.value?'PATCH':'POST',body:JSON.stringify(fieldPayload(body))});if(request!==loadVersion)return;items.value=[...items.value.filter(field=>field.id!==result.id),result].sort((a,b)=>a.sortOrder-b.sortOrder);opened.value=false;notice.value='字段配置已保存'}
...
async function toggle(field:Field) {
  ...
  try{const result=await scope.request<Field>('/field-definitions/'+field.id,{method:'PATCH',body:JSON.stringify(fieldPayload({...field,enabled:!field.enabled}))});if(request===loadVersion)items.value=items.value.map(item=>item.id===result.id?result:item)}catch(cause){if(request===loadVersion&&scope.current())error.value=message(cause)}finally{if(request===loadVersion)saving.value=false}
}
