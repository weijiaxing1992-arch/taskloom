...
import { sprintParticipants, sprintTeamParticipation } from '../sprintPeople'
import type { SprintWeightSummary } from '../sprintWeights'
import{Teleport,computed,nextTick,onBeforeUnmount,onMounted,reactive,ref,watch}from'vue';import{useRoute,useRouter}from'vue-router';import{api}from'../api'
...
const projectHeaders=()=>({'X-TaskLoom-Project':pageProject})
...
// 迭代工作项默认先列需求、后列缺陷。业务类型优先级由 queryWorkItems 统一
// 定义，点击类型列仍可反向查看缺陷，其他字段排序也完全保持可用。
const fieldDefinitions=ref<any[]>([]),listRules=ref<WorkFilter[]>([]),listSearch=ref(''),listSort=ref('objectType'),listOrder=ref('asc'),listPage=ref(1),listColumns=ref<string[]>([]),columnsSaving=ref(false),columnsError=ref(''),columnPanel=ref<InstanceType<typeof WorkItemColumns>|null>(null)
...
const sprints=ref<any[]>([]),members=ref<any[]>([]),selected=ref<any>(null),backlog=ref<any[]>([]),activeTab=ref('工作项列表'),showSprint=ref(false),showRequirementComposer=ref(false),showDefectComposer=ref(false),showComplete=ref(false),error=ref(''),completionTarget=ref('待规划'),loading=ref(true),saving=ref(false)
// 创建迭代遮罩必须脱离 main 的滚动和路由动画层。首屏或独立组件测试没有宿主时
// 先原位渲染，挂载后再启用 Teleport，避免找不到目标节点。
const overlayTargetReady=ref(false)
const SprintOverlayHost=computed(()=>overlayTargetReady.value?Teleport:'div')
const requirementComposer=ref<{requestClose:()=>Promise<boolean>}|null>(null)
const releaseNotesTarget=ref<{id:number;name:string}|null>(null)
function openReleaseNotes(){if(currentPage()&&!saving.value&&!selected.value?.backlog&&selected.value?.sprint?.status==='已完成')releaseNotesTarget.value={id:selected.value.sprint.id,name:selected.value.sprint.name}}
...
const normalizedSprintSearch=computed(()=>sprintSearch.value.trim().toLocaleLowerCase())
function sprintMatchesSearch(sprint:any){
 const query=normalizedSprintSearch.value
 if(!query)return true
 return [sprint?.name,sprint?.code,sprint?.status,sprint?.goal,sprint?.startDate,sprint?.endDate]
  .filter(value=>value!==undefined&&value!==null)
  .some(value=>String(value).toLocaleLowerCase().includes(query))
}
const activeSprints=computed(()=>sprints.value.filter(sprint=>!['已完成','已取消'].includes(sprint.status)&&sprintMatchesSearch(sprint)))
const historicalSprints=computed(()=>sprints.value.filter(sprint=>['已完成','已取消'].includes(sprint.status)&&sprintMatchesSearch(sprint)))
...
// 表格行本身也是快捷入口，但不能截获编号、标题、链接或内联输入框的原有行为。
// 这让鼠标点击空白单元格和键盘聚焦整行都能打开工作项，同时保留复制编号、编辑工时等操作。
function isWorkItemControlEvent(event:Event):boolean{
 const boundary=event.currentTarget
 let target=event.target as Element|null
 while(target&&target!==boundary){
  if(typeof target.matches==='function'&&target.matches('a,button,input,select,textarea,label,summary,[contenteditable="true"],[role="button"],[role="menuitem"],[role="option"],[data-work-item-interactive]'))return true
  target=target.parentElement
 }
 return false
}
function workItemRowLabel(x:any){
 const type=t(x.objectType==='defect'?'缺陷':'需求'),code=String(x.code||'').trim(),title=String(x.title||'').trim()
 return [type,code,title].filter(Boolean).join(' · ')
}
async function openWorkItem(x:any,event?:Event){
 if(!currentPage()||!x||!Number.isSafeInteger(Number(x.id))||Number(x.id)<1||event&&isWorkItemControlEvent(event))return
 const trigger=event?.currentTarget
 if(x.objectType==='requirement'){await openRequirement(x,trigger);return}
 if(x.objectType==='defect'){
  try{await router.push(workItemLink(x))}catch(cause){if(currentPage())error.value=errorMessage(cause)}
 }
}
async function openRequirement(x:any,trigger?:EventTarget|null){
 if(!currentPage()||x.objectType!=='requirement'||!Number.isSafeInteger(Number(x.id))||Number(x.id)<1||!selected.value)return
 requirementTrigger=trigger instanceof HTMLElement?trigger:document.activeElement instanceof HTMLElement?document.activeElement:null
...
async function openRequirementCreator(){
 if(!currentPage()||saving.value||!canCreateWork.value||!selected.value)return
 error.value='';showRequirementComposer.value=true
}
async function closeRequirementCreator(){
 if(!showRequirementComposer.value)return
 const closed=await requirementComposer.value?.requestClose()
 if(closed)showRequirementComposer.value=false
}
async function requirementCreated(){
 if(!currentPage())return
 showRequirementComposer.value=false
 const sprintId=Number(selected.value?.sprint?.id)
 const preferredId=!selected.value?.backlog&&Number.isSafeInteger(sprintId)&&sprintId>0?sprintId:undefined
 await load(preferredId)
}
function openDefectCreator(){
 if(!currentPage()||saving.value||!canCreateWork.value||!selected.value)return
 error.value='';showDefectComposer.value=true
}
async function defectCreated(){
 if(!currentPage())return
 showDefectComposer.value=false
 const sprintId=Number(selected.value?.sprint?.id)
 const preferredId=!selected.value?.backlog&&Number.isSafeInteger(sprintId)&&sprintId>0?sprintId:undefined
 await load(preferredId)
}
...
async function complete(){
 const sprint=selected.value?.sprint
 if(!currentPage()||saving.value||!canManage.value||!sprint?.id||sprint.status!=='进行中')return
 const id=sprint.id
 error.value='';saving.value=true
 try{
  // Completion only waits for its transaction; any automatic AI job runs server-side.
  await api('/sprints/'+id+'/complete',{method:'POST',headers:projectHeaders(),body:JSON.stringify({targetSprint:completionTarget.value})})
  if(!currentPage()||selected.value?.sprint?.id!==id)return
  showComplete.value=false
  await load(id)
  if(currentPage()&&selected.value?.sprint?.id===id&&selected.value.sprint.status==='已完成'){
   // 自动生成开启时立即展示后台任务；未开启时不打断完成迭代的主流程。
   try{const release=await api<any>('/sprints/'+id+'/release-notes',{headers:projectHeaders()});if(currentPage()&&selected.value?.sprint?.id===id&&['queued','generating','draft','failed'].includes(String(release?.state)))releaseNotesTarget.value={id,name:selected.value.sprint.name}}catch{/* 迭代已完成，日志状态读取失败不应把成功操作显示成失败。 */}
  }
 }catch(cause){if(currentPage()&&selected.value?.sprint?.id===id)error.value=errorMessage(cause)}
 finally{if(currentPage())saving.value=false}
}
onMounted(async()=>{overlayTargetReady.value=typeof document.getElementById==='function'&&!!document.getElementById('workspace-overlays');await load(route.query.sprint&&route.query.sprint!=='backlog'?Number(route.query.sprint):undefined);await loadColumns()})
...
watch(()=>route.query.sprint,value=>{releaseNotesTarget.value=null;if(value==='backlog'){if(!selected.value?.backlog)void load()}else if(value){if(Number(value)!==selected.value?.sprint.id)void load(Number(value))}else if(!route.query.req)void load()})
...
<template><div class="iteration-shell" :inert="!!route.query.req||!!releaseNotesTarget||showDefectComposer"><aside class="iteration-nav" :class="{'is-collapsed':!iterationSidebarExpanded}"><SidebarCollapseButton :expanded="iterationSidebarExpanded" :label="t('迭代侧边栏')" @toggle="iterationSidebarExpanded=!iterationSidebarExpanded"/><div v-show="iterationSidebarExpanded" class="iteration-nav-content"><header><div><span class="eyebrow">{{ t("交付节奏") }}</span><h2>{{ t("迭代") }}</h2></div><button v-if="canManage" type="button" class="create-sprint-button" :disabled="saving" @click="openSprint"><span aria-hidden="true">＋</span>{{t('创建迭代')}}</button></header><label class="iteration-search"><span class="sr-only">{{t('搜索迭代')}}</span><span aria-hidden="true">⌕</span><input v-model="sprintSearch" :aria-label="t('搜索迭代')" :placeholder="t('搜索迭代名称、编号或状态')"></label><button v-if="sprintSearch" type="button" class="iteration-search-clear" @click="sprintSearch=''">{{t('清除')}}</button><button class="backlog-link" :class="{active:selected?.backlog}" :disabled="saving||loading" @click="navigateSprint('backlog')"><span>☷</span><b>{{ t("待规划") }}</b><em>{{backlog.length}}</em></button><div class="iteration-label">{{ t("进行中 / 规划中") }}</div><p v-if="!loading&&!sprints.length" class="empty-mini">{{ t("暂无迭代，可先在待规划中整理工作项。") }}</p><p v-else-if="!activeSprints.length" class="empty-mini">{{t('没有匹配的进行中或规划中迭代')}}</p><template v-for="(x,index) in activeSprints" :key="x.id"><button class="iteration-card" :disabled="saving" :class="{active:selected?.sprint?.id===x.id}" @click="navigateSprint(x.id)"><div><b>{{x.name}}</b><span :class="['status','workflow-color',x.status]" :style="workflowStyle(x.status)">{{ t(x.status) }}</span></div><small>{{formatDate(x.startDate)}} — {{formatDate(x.endDate)}}</small><div class="nav-progress"><i :style="{width:sprintPct(x)+'%'}"></i></div><footer><span>{{x.done}} / {{x.total}} {{ t("项") }}</span><strong>{{sprintPct(x)}}%</strong></footer></button><div v-if="index<activeSprints.length-1" class="iteration-separator" aria-hidden="true"></div></template><div class="iteration-label">{{ t("历史迭代") }}</div><p v-if="sprints.some(x=>['已完成','已取消'].includes(x.status))&&!historicalSprints.length" class="empty-mini">{{t('没有匹配的历史迭代')}}</p><template v-for="(x,index) in historicalSprints" :key="x.id"><button class="iteration-card compact-card" :disabled="saving" :class="{active:selected?.sprint?.id===x.id}" @click="navigateSprint(x.id)"><div><b>{{x.name}}</b><span :class="['status','workflow-color',x.status]" :style="workflowStyle(x.status)">{{ t(x.status) }}</span></div><small>{{x.done}} / {{x.total}} {{ t("项 ·") }} {{sprintPct(x)}}%</small></button><div v-if="index<historicalSprints.length-1" class="iteration-separator" aria-hidden="true"></div></template></div></aside>
<section class="iteration-main" v-if="selected&&!loading"><header class="iteration-head compact-page-heading"><div><div class="iteration-kicker"><span>{{selected.sprint.code}}</span><span :class="['status','workflow-color',selected.sprint.status]" :style="workflowStyle(selected.sprint.status)">{{t(selected.sprint.status||"待规划")}}</span></div><h1 :title="selected.backlog ? t(selected.sprint.name) : selected.sprint.name">{{selected.backlog ? t(selected.sprint.name) : selected.sprint.name}}</h1><details v-if="selected.sprint.goal" class="page-heading-note iteration-goal"><summary :aria-label="t('迭代目标')" :title="t('迭代目标')"><span aria-hidden="true">?</span></summary><p>{{selected.backlog ? t(selected.sprint.goal) : selected.sprint.goal}}</p></details></div><div class="iteration-actions"><button v-if="!selected.backlog&&selected.sprint.status==='已完成'" class="btn" :disabled="saving" @click="openReleaseNotes">{{t('升级日志')}}</button><button v-if="canManage&&selected.sprint.status==='规划中'" class="btn" :disabled="saving" @click="patchSprint({status:'进行中'})">{{ t("开始迭代") }}</button><button v-if="canManage&&selected.sprint.status==='进行中'" class="btn danger-outline" :disabled="saving" @click="patchSprint({status:'已取消'})">{{ t("取消迭代") }}</button><button v-if="canManage&&selected.sprint.status==='进行中'" class="btn" :disabled="saving" @click="error='';completionTarget='待规划';showComplete=true">{{ t("完成迭代") }}</button><button v-if="canWrite" class="btn primary" :disabled="!canCreateWork||saving" :title="t(canCreateWork?'打开完整需求创建页':'历史迭代不能新增需求')" @click="openRequirementCreator">{{ t("＋ 创建需求") }}</button><button v-if="canWrite" class="btn" :disabled="!canCreateWork||saving" :title="t(canCreateWork?'打开完整缺陷创建页':'历史迭代不能新增缺陷')" @click="openDefectCreator">{{ t("＋ 创建缺陷") }}</button></div></header><div v-if="error&&!showSprint&&!showDefectComposer&&!showComplete" class="readonly-banner" role="alert">{{ t(error) }}</div><div v-if="!selected.backlog&&['已完成','已取消'].includes(selected.sprint.status)" class="readonly-banner">{{ t("此迭代已结束，保留历史工作项；新增需求或缺陷请放入待规划或开放迭代。") }}</div><SprintTabs v-model="activeTab" v-model:order="tabs"><template #工作项列表><small>{{metrics.total}}</small></template></SprintTabs>
...
<component :is="SprintOverlayHost" v-bind="overlayTargetReady?{to:'#workspace-overlays'}:{}"><div v-if="showSprint" class="modal-shade slide-panel-shade" @click.self="!saving&&(showSprint=false)"><div class="modal"><header><h2>{{ t("创建迭代") }}</h2><button :disabled="saving" @click="showSprint=false" :aria-label="t('关闭')">×</button></header><div class="modal-body form-grid"><label>{{ t("迭代名称 *") }}</label><input :aria-label="t('迭代名称 *')" v-model="sprintForm.name" @input="sprintCustomized.name=true"><label>{{ t("迭代目标") }}</label><textarea :aria-label="t('迭代目标')" v-model="sprintForm.goal"></textarea><label>{{ t("开始日期 *") }}</label><DatePicker :label="t('开始日期 *')" :model-value="sprintForm.startDate" required :disabled="saving" @update:model-value="changeSprintStart" @validity-change="sprintDateValidity.start=$event" /><label>{{ t("结束日期 *") }}</label><div><DatePicker :label="t('结束日期 *')" v-model="sprintForm.endDate" required :min="sprintForm.startDate" :disabled="saving" @input="sprintCustomized.endDate=true" @change="sprintCustomized.endDate=true" @validity-change="sprintDateValidity.end=$event" /><p class="hint">{{t('本周五至下周四，名称和结束日期可手动调整')}}</p></div><label>{{ t("容量（小时）") }}</label><input :aria-label="t('容量（小时）')" type="number" min="0" v-model="sprintForm.capacity"><p v-if="error" class="field-error">{{ t(error) }}</p></div><footer><button class="btn" :disabled="saving" @click="showSprint=false">{{ t("取消") }}</button><button class="btn primary" :disabled="saving" @click="createSprint">{{saving?t("创建中…"):t("创建迭代")}}</button></footer></div></div></component>
<DefectComposer v-if="showDefectComposer" :initial-sprint="selected?.backlog?'':selected?.sprint?.name" @created="defectCreated" @cancel="showDefectComposer=false"/>
<component :is="SprintOverlayHost" v-bind="overlayTargetReady?{to:'#workspace-overlays'}:{}">
<div v-if="showRequirementComposer" class="drawer-shade sprint-requirement-shade" @click.self="closeRequirementCreator"><section class="drawer sprint-requirement-drawer" role="dialog" aria-modal="true" :aria-label="t('创建需求')"><Editor ref="requirementComposer" embedded :initial-sprint="selected?.backlog?'':selected?.sprint?.name" @created="requirementCreated" @cancel="showRequirementComposer=false"/></section></div>
<div v-if="showComplete" class="modal-shade" @click.self="!saving&&(showComplete=false)"><div class="modal complete-modal"><header><h2>{{ t("完成迭代") }}</h2><button :disabled="saving" @click="showComplete=false" :aria-label="t('关闭')">×</button></header><div class="modal-body"><div class="completion-summary"><b>{{metrics.unfinished}}</b><span>{{ t("个未完成工作项需要迁移") }}</span></div><label>{{ t("迁移目标") }}</label><select :aria-label="t('迁移目标')" v-model="completionTarget"><option value="待规划">{{ t("待规划") }}</option><option v-for="x in sprints.filter(x=>x.id!==selected.sprint.id&&['规划中','进行中'].includes(x.status))" :value="x.name">{{x.name}}</option></select><p class="hint">{{ t("完成操作会在同一事务中迁移未完成需求和缺陷，并写入迭代活动与审计日志。") }}</p><p v-if="error" class="field-error">{{ t(error) }}</p></div><footer><button class="btn" :disabled="saving" @click="showComplete=false">{{ t("暂不完成") }}</button><button class="btn primary" :disabled="saving" @click="complete">{{saving?t("处理中…"):t("确认完成")}}</button></footer></div></div>
</component>
</div><Requirements v-if="route.query.req" detail-only @updated="refreshRequirement"/><ReleaseNotesPanel v-if="releaseNotesTarget" :project-id="pageProject" :sprint-id="releaseNotesTarget.id" :sprint-name="releaseNotesTarget.name" @close="releaseNotesTarget=null"/></template>
<style scoped>
.sprint-requirement-shade{z-index:1250;isolation:isolate;background:color-mix(in srgb,#15233a 30%,transparent);backdrop-filter:blur(2px);-webkit-backdrop-filter:blur(2px)}.sprint-requirement-drawer{width:min(1280px,92vw);max-width:100%;height:100%;max-height:100%;overflow:hidden}.sprint-requirement-drawer :deep(.requirement-editor){height:100%;min-height:0}.sprint-requirement-drawer :deep(.editor-split){min-height:0}
...
.iteration-search{display:flex;align-items:center;gap:7px;min-height:34px;margin:0 8px 8px;padding:0 9px;border:1px solid color-mix(in srgb,var(--border) 86%,var(--background));border-radius:8px;background:var(--card);color:var(--muted-foreground);transition:border-color .15s ease,box-shadow .15s ease}.iteration-search:focus-within{border-color:color-mix(in srgb,var(--primary) 64%,var(--border));box-shadow:0 0 0 3px color-mix(in srgb,var(--primary) 12%,transparent)}.iteration-search>span{font-size:17px;line-height:1}.iteration-search input{min-width:0;flex:1;width:100%;border:0!important;outline:0;background:transparent!important;color:var(--foreground);font:inherit;font-size:12px}.iteration-search input::placeholder{color:var(--muted-foreground)}.iteration-search-clear{align-self:flex-end;margin:-4px 9px 5px;padding:2px 3px;border:0;background:transparent;color:var(--primary);font:inherit;font-size:11px;cursor:pointer}.iteration-search-clear:hover{text-decoration:underline}.iteration-search-clear:focus-visible{outline:2px solid var(--ring,var(--primary));outline-offset:2px;border-radius:3px}.iteration-separator{height:1px;margin:1px 12px;background:color-mix(in srgb,var(--border) 72%,transparent)}.sr-only{position:absolute;width:1px;height:1px;padding:0;margin:-1px;overflow:hidden;clip:rect(0,0,0,0);white-space:nowrap;border:0}
...
.work-item-type{display:inline-flex;align-items:center;min-height:24px;padding:2px 8px;border:1px solid transparent;border-radius:999px;font-size:12px;font-weight:650;line-height:1.35;white-space:nowrap}.work-item-type-requirement{color:color-mix(in srgb,#2563eb 84%,var(--foreground));background:color-mix(in srgb,#2563eb 11%,var(--card));border-color:color-mix(in srgb,#2563eb 24%,var(--border))}.work-item-type-defect{color:color-mix(in srgb,#c2410c 84%,var(--foreground));background:color-mix(in srgb,#ea580c 11%,var(--card));border-color:color-mix(in srgb,#ea580c 24%,var(--border))}
...
.work-item-row{cursor:pointer}.work-item-row:focus-visible>td{position:relative;box-shadow:inset 0 2px 0 var(--primary),inset 0 -2px 0 var(--primary)}.work-item-row:focus-visible>td:first-child{box-shadow:inset 2px 0 0 var(--primary),inset 0 2px 0 var(--primary),inset 0 -2px 0 var(--primary)}.work-item-row:focus-visible>td:last-child{box-shadow:inset -2px 0 0 var(--primary),inset 0 2px 0 var(--primary),inset 0 -2px 0 var(--primary)}
...
@media(max-width:820px){
 ...
 .iteration-nav.is-collapsed{width:100%;min-width:0;max-height:none;padding:9px}.iteration-nav.is-collapsed>:deep(.sidebar-collapse-toggle){margin-left:auto}.iteration-search{margin-inline:4px}.iteration-search-clear{margin-right:4px}
