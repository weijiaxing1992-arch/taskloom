...
export async function readTapdPdf(file: File, signal: AbortSignal, progress: (message:string)=>void) {
  ...
  try {
    ...
    const pages:TapdPage[]=[], images:string[]=[];let imageBytes=0
    for(let n=1;n<=doc.numPages;n++){
      ...
      pages.push({width:viewport.width,height:viewport.height,items:text.items.filter((x):x is typeof x & PdfText=>'str'in x)})
      const scale=Math.min(1.8,1600/Math.max(viewport.width,viewport.height)),renderView=page.getViewport({scale})
      const canvas=document.createElement('canvas');canvas.width=Math.ceil(renderView.width);canvas.height=Math.ceil(renderView.height)
      const context=canvas.getContext('2d');if(!context)throw Error('当前浏览器无法生成 PDF 页面预览')
      await page.render({canvasContext:context,canvas,viewport:renderView}).promise
      const data=canvas.toDataURL('image/jpeg',.86);imageBytes+=data.length
      canvas.width=0;canvas.height=0
      if(imageBytes>10*1024*1024)throw Error('页面预览超过大小限制，请拆分 PDF 后重试')
      images.push(data);page.cleanup()
    }
    ...
    return {...parsed,images,pageCount:doc.numPages,pdfBase64:btoa(binary),fileName:file.name}
