...
await test('bound defect creation uses the full active iteration and stable assignee/verifier IDs',async()=>{
 ...
 assert.equal(sent.path,'/defects');assert.equal(body.requirementId,17);assert.equal(body.assigneeUserId,'dev');assert.equal(body.assignee,'同名');assert.equal(body.verifierUserId,'qa');assert.equal(body.verifier,'同名');assert.equal(m.events[0].item.requirementId,17);assert.equal(c.dirty,false);assert(m.calls.every(call=>call.options.headers.get('X-DevFlow-Project')==='project-a'));assert.equal(m.calls.some(call=>call.options.method==='PATCH'),false);m.stop()
})
...
await test('requirement picker searches by literal code/title with stable IDs, scoped headers and keyboard selection',async()=>{
 const m=await mount('RequirementPicker',{props:{modelValue:null}}),c=m.context;c.open();await flush();c.query='REQ-19 / 用户标题';await c.search();await flush();assert.equal(new URLSearchParams(m.calls.at(-1).path.split('?')[1]).get('q'),'REQ-19 / 用户标题');assert.equal(m.calls.at(-1).options.headers.get('X-DevFlow-Project'),'project-a');c.highlighted=2;c.keydown({key:'Enter',preventDefault(){}});assert.deepEqual(m.events.find(event=>event.type==='selected'),{type:'selected',id:19});assert.equal(c.opened,false);assert.equal(c.query,'');m.stop()
})
