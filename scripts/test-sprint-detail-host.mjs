...
const richText=evaluate(await read('src/richText.ts'),{'./mentions':mentions}),memberRoles=evaluate(await read('src/memberRoles.ts'))
...
class Element {constructor(tag,text=''){Object.assign(this,{tag,text,props:{},children:[],parent:null,isConnected:true,focused:false,open:false})}focus(options){this.focused=true;this.focusOptions=options}showModal(){assert.equal(this.tag,'dialog');this.open=true;this.props.open=''}close(){assert.equal(this.tag,'dialog');this.open=false;delete this.props.open}checkValidity(){return true}reportValidity(){return true}contains(value){return this===value||this.children.some(child=>child.contains(value))}closest(selector){return selector.includes('inert')&&this.props.inert?this:this.parent?.closest(selector)||null}getClientRects(){return[{}]}querySelectorAll(){return this.children.flatMap(descendants).filter(node=>['button','input','select','textarea'].includes(node.tag)&&!node.props.disabled)}}
...
async function mount({query={},override=()=>undefined,layoutStorage=new Map()}={}){
  ...
  imports['../richText']=richText;imports['../memberRoles']=memberRoles
  ...
  for(const name of ['Editor','RequirementLinks','DefectComposer','Requirements','SprintTabs','Sprints']){
    ...
    // Match a Vue module namespace for both static imports and CommonJS-
    // transpiled import(). Without __esModule, __importStar nests default twice.
    const resolve=id=>{if(id.endsWith('.vue')){const child=id.split('/').at(-1).slice(0,-4);return{__esModule:true,default:components[child]||stub(child)}}assert(id in imports,'Unexpected '+id);return imports[id]}
  ...
  const router=Router.createRouter({history:Router.createMemoryHistory(),routes:[{path:'/iterations',component:components.Sprints},{path:'/requirements',component:stub('OtherPage')},{path:'/requirements/new',component:stub('RequirementEditor')},{path:'/defects',component:stub('DefectsPage')}]})
...
await test('clicking a requirement opens the real current-page host without reloading list or clearing filters',async()=>{
  ...
  // The child lookup uses the lightweight reference projection; it is not a
  // paginated pool reload and must not fetch every requirement's rich body.
  assert.equal(m.calls.slice(calls).some(call=>call.path==='/sprints/1'),false);assert.equal(m.calls.slice(calls).some(call=>call.path.startsWith('/requirements?')&&call.path!=='/requirements?projection=reference'||call.path==='/preferences/requirement-list'),false);assert.equal(m.calls.slice(calls).filter(call=>call.path==='/requirements?projection=reference').length,1);assert.equal(m.context('Requirements').selected.id,17);assert.equal(descendants(m.container).some(node=>String(node.props.class||'').includes('requirements-sidebar')),false);assert.equal(descendants(m.container).find(node=>node.props.class==='iteration-shell').props.inert,true);m.stop()
})
await test('safe work-item row cells and keyboard entry open details without hijacking controls',async()=>{
 const m=await mount({query:{sprint:'1',release:'keep'}}),s=m.context('Sprints'),row=new Element('tr'),plain={matches:()=>false,parentElement:row},control={matches:selector=>selector.includes('button'),parentElement:row}
 await s.openWorkItem({...initialReq(17),objectType:'requirement'},{target:plain,currentTarget:row});await flush()
 assert.deepEqual({...m.router.currentRoute.value.query},{sprint:'1',release:'keep',req:'17'});assert.equal(m.context('Requirements').selected.id,17)
 m.context('Requirements').close();await flush();assert.equal(row.focused,true);assert.equal(row.focusOptions.preventScroll,true)
 await s.openWorkItem({...initialReq(17),objectType:'requirement'},{target:control,currentTarget:row});await flush();assert.equal(m.router.currentRoute.value.query.req,undefined)
 assert.match(sources.Sprints,/@keydown\.enter\.self\.prevent="openWorkItem\(x,\$event\)"/);assert.match(sources.Sprints,/@keydown\.space\.self\.prevent="openWorkItem\(x,\$event\)"/);assert.match(sources.Sprints,/function isWorkItemControlEvent/)
 m.stop()
})
await test('safe defect row cells keep the existing defect route behavior',async()=>{
 const m=await mount(),s=m.context('Sprints'),row=new Element('tr'),plain={matches:()=>false,parentElement:row}
 await s.openWorkItem({...initialReq(71),objectType:'defect',code:'BUG-0071'},{target:plain,currentTarget:row});await flush()
 assert.equal(m.router.currentRoute.value.path,'/defects');assert.equal(m.router.currentRoute.value.query.bug,'71');m.stop()
})
await test('iteration shortcuts open a requirement drawer and the defect composer with the selected sprint',async()=>{
 const requirement=await mount(),s=requirement.context('Sprints');await s.openRequirementCreator();await flush()
 assert.equal(s.showRequirementComposer,true);assert.equal(requirement.router.currentRoute.value.path,'/iterations');assert.match(sources.Sprints,/<div v-if="showRequirementComposer" class="drawer-shade sprint-requirement-shade"/);assert.match(sources.Sprints,/:initial-sprint="selected\?\.backlog\?'':selected\?\.sprint\?\.name"/);requirement.stop()
 const defect=await mount(),sprint=defect.context('Sprints');sprint.openDefectCreator();await flush()
 const composer=defect.context('DefectComposer');assert(composer);assert.equal(composer.form.sprint,'123');assert.match(sources.Sprints,/openRequirementCreator/);assert.match(sources.Sprints,/openDefectCreator/);assert.match(sources.Sprints,/<DefectComposer v-if="showDefectComposer"/);assert.doesNotMatch(sources.Sprints,/创建工作项/);defect.stop()
})
await test('iteration navigator searches names, codes and status while retaining subtle card separators',async()=>{
 const m=await mount(),s=m.context('Sprints');s.sprintSearch='SPR-2';await flush();assert.deepEqual(s.activeSprints.map(row=>row.id),[2]);s.sprintSearch='does-not-exist';await flush();assert.equal(s.activeSprints.length,0);assert.match(sources.Sprints,/class="iteration-separator"/);assert.match(sources.Sprints,/搜索迭代名称、编号或状态/);m.stop()
})
...
await test('full editor cancellation keeps unsaved values and saving blocks parent route changes',async()=>{
  const pending=deferred(),m=await mount({query:{sprint:'1',req:'17'},override:(path,options)=>options?.method==='PATCH'?pending.promise:undefined}),r=m.context('Requirements');await r.editFull();await flush();const e=m.context('Editor');e.f.title='Do not lose';const closing=e.requestClose();await flush();const dialog=descendants(m.container).find(node=>node.tag==='dialog'&&node.open);assert(dialog,'dirty editor must open its real leave dialog');const keepEditing=descendants(dialog).find(node=>node.tag==='button'&&text(node)==='继续编辑');assert(keepEditing);keepEditing.props.onClick();assert.equal(await closing,false);assert.equal(dialog.open,false);assert.equal(e.f.title,'Do not lose');const saving=e.save();await flush();const result=await m.router.replace({path:'/iterations',query:{sprint:'2'}});assert(Router.isNavigationFailure(result));assert.equal(m.router.currentRoute.value.query.req,'17');pending.reject(Error('save failed'));await saving;await flush();assert.equal(e.error,'save failed');assert.equal(e.f.title,'Do not lose');assert.equal(r.fullEditorID,17);m.stop()
})
...
await test('iteration title, board, weight detail and code entries all route through the shared host',async()=>{
  assert.match(sources.Sprints,/<Requirements v-if="route.query.req" detail-only @updated="refreshRequirement"/);assert.match(sources.Sprints,/@open-requirement="openRequirement"/);assert.match(sources.Sprints,/if\(x\.objectType==='requirement'\)\{await openRequirement/);assert(!/iframe/i.test(sources.Requirements));assert.match(sources.Requirements,/if\(!props.detailOnly\)await Promise.all\(\[load\(\),loadColumns\(\)\]\)/);assert.match(await read('src/components/SprintWeightPanel.vue'),/<RequirementCode/)
})
