...
await test('readonly scope bundle is mandatory, write scopes fail closed and expiry/name are bounded', () => {
  ...
  assert.deepEqual(helpers.credentialRequest('Notifications', ['notifications:read'], 30, readonly).scopes, [...helpers.contextReadScopes, 'notifications:read'])
...
await test('viewer creates only a personal read credential, with secret kept out of config and metadata', async () => {
  ...
  assert.deepEqual(JSON.parse(creation.options.body).scopes, [...helpers.contextReadScopes]); assert.equal(creation.options.headers.get('X-TaskLoom-Project'), 'p-current')
...
await test('template compiles, navigation excludes impersonation, and page never stores or interpolates secrets into URLs', () => {
  ...
  assert.match(source, /本人通知只返回当前项目的个人收件箱且不会自动标为已读/)
