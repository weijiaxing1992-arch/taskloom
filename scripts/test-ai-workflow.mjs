...
function fixture(kind, handler = async (path, options) => kind === 'settings' ? settings() : options ? preview() : capability()) {
 ...
 const common = 'loading,saving,error,notice,dirty,load,canLeave', names = common + (kind === 'settings' ? ',settings,apiKey,model,enabled,disabled,save,remove,beforeProjectChange,cancelProjectLeave,baseUrl,endpointMode,effectiveBaseUrl,requestAddress,discard' : ',capability,draft,selected,consent,imported,stale,expired,canGenerate,canImport,generating,importing,focus,extraInstructions,count,importDestination,supportedFocus,countOptions,generate,toggle,selectAll,importCases,discard')
...
await test('custom API URL normalizes a root once and retains an existing key only after explicit destination confirmation',async()=>{
 let saved=settings({baseUrl:'https://api.openai.com/v1',version:2,endpointMode:'default'})
 const m=fixture('settings',async(path,options)=>{if(options){const patch=JSON.parse(options.body);saved={...saved,baseUrl:patch.baseUrl,endpointMode:'custom',version:3}}return saved})
 await m.load();m.endpointMode.value='custom';m.baseUrl.value=' https://api.owlai.tech/ ';assert.equal(m.requestAddress.value,'https://api.owlai.tech/v1/responses');assert.equal(m.dirty.value,true)
 m.setConfirm(false);await m.save();assert.equal(m.calls.length,1);assert.equal(m.baseUrl.value,' https://api.owlai.tech/ ')
 m.setConfirm(true);await m.save();assert.deepEqual(JSON.parse(m.calls[1].options.body),{expectedVersion:2,baseUrl:'https://api.owlai.tech/v1',reuseKey:true});assert.equal(m.apiKey.value,'');assert.equal(m.dirty.value,false);assert.equal(m.requestAddress.value,'https://api.owlai.tech/v1/responses')
 m.baseUrl.value='https://api.owlai.tech/v1/';assert.equal(m.dirty.value,false);m.discard();assert.equal(m.baseUrl.value,'https://api.owlai.tech/v1');m.stop()
})
await test('invalid URLs and outdated backends cannot send a key; replacement keys bind with their selected destination',async()=>{
 const m=fixture('settings',async()=>settings({version:4}));await m.load();m.endpointMode.value='custom';m.apiKey.value='new-test-provider-key'
 for(const value of ['http://api.owlai.tech','https://name:secret@api.owlai.tech','https://api.owlai.tech?key=secret','https://api.owlai.tech#secret','https://api.owlai.tech\\evil','not-a-url']){m.baseUrl.value=value;await m.save();assert.equal(m.calls.length,1);assert.equal(m.apiKey.value,'new-test-provider-key')}
 m.baseUrl.value='https://api.owlai.tech/custom/v1';await m.save();assert.deepEqual(JSON.parse(m.calls[1].options.body),{apiKey:'new-test-provider-key',expectedVersion:4,baseUrl:'https://api.owlai.tech/custom/v1'});assert.equal(m.confirmations.length,0)
 m.settings.value=settings();m.endpointMode.value='custom';m.baseUrl.value='https://api.owlai.tech';const count=m.calls.length;await m.save();assert.equal(m.calls.length,count);assert.match(m.error.value,/升级服务端/);m.stop()
})
await test('URL change failures retain the draft and identity switches clear both URL and key',async()=>{
 const m=fixture('settings',async(path,options)=>{if(options)throw Error('AI 配置已被其他管理员修改，请刷新后重新确认');return settings({baseUrl:'https://api.owlai.tech/v1',endpointMode:'custom',version:3})})
 await m.load();m.endpointMode.value='default';await m.save();assert.equal(m.calls.length,2);assert.deepEqual(JSON.parse(m.calls[1].options.body),{expectedVersion:3,baseUrl:'https://api.openai.com/v1',reuseKey:true});assert.equal(m.endpointMode.value,'default');assert.equal(m.settings.value.baseUrl,'https://api.owlai.tech/v1');assert.match(m.error.value,/其他管理员/)
 m.apiKey.value='draft-only';m.locked.value=true;assert.equal(m.apiKey.value,'');assert.equal(m.baseUrl.value,'');assert.equal(m.settings.value,null);m.stop()
})
await test('even key-only edits and removals carry the loaded settings version and explicit key destination',async()=>{
 const initial=settings({baseUrl:'https://api.owlai.tech/v1',endpointMode:'custom',version:5}),m=fixture('settings',async()=>initial);await m.load();m.apiKey.value='provider-key-replacement';await m.save()
 assert.deepEqual(JSON.parse(m.calls[1].options.body),{expectedVersion:5,apiKey:'provider-key-replacement',baseUrl:'https://api.owlai.tech/v1'});m.enabled.value=false;await m.save();assert.deepEqual(JSON.parse(m.calls[2].options.body),{expectedVersion:5,enabled:false})
 await m.remove();assert.deepEqual(JSON.parse(m.calls[3].options.body),{clear:true,expectedVersion:5});m.stop()
})
